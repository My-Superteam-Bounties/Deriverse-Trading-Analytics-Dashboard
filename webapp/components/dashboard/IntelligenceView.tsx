...
import { IntelligenceDrawer } from "@/components/chat/IntelligenceDrawer";
...
export function IntelligenceView({ onSwitchToTerminal }: IntelligenceViewProps) {
    const { input, setInput, submitQuery, openDrawer } = useChatStore();
    ...
    return (
        <WalletGuard>
            <div className={cn(
                "w-full mx-auto transition-all duration-500 min-h-[80vh] flex flex-col items-center justify-center gap-16"
            )}>
                {/* 1. Hero Content (Always visible as dashboard) */}
                    ...
                    <div className="w-full max-w-2xl relative group mx-auto mt-12">
                        ...
                        <div id="tour-intelligence-input" className="relative bg-card border border-border rounded-2xl p-2 flex items-center shadow-2xl focus-within:ring-2 focus-within:ring-primary/20 transition-all">
                            ...
                            <input
                                ...
                                onKeyDown={(e) => e.key === "Enter" && handleHeroSubmit(input)}
                            ...
                            />
                            <button
                                onClick={() => handleHeroSubmit(input)}
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-3 mt-8">
                            ...
                            {[
                                ...
                                <button
                                    key={i}
                                    onClick={() => handleHeroSubmit(suggestion.label)}
                ...
                {/* 2. Drawer Interface (Always rendered, state controlled internally) */}
                <IntelligenceDrawer />
            </div>
        </WalletGuard>
    );
